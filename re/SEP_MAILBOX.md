...
### КОРЕНЬ (полная цепочка): транзакция мейлбокса не «завершается» → паника → reboot
Первый фатал — в коде мейлбокса SEP (TB перед первой паникой: 0x106cd0/0x1073b8/0x107080/0x107458).
Панику даёт FUN_00104d68/FUN_00104d94: `SVC #0xa0` ×2 + FUN_0010550c(0x84000000/0x84000009)
= PSCI_VERSION+SYSTEM_RESET (ровно наши два HVC). SVC#0xa0 обрабатывает FUN_00114400 (читает
esr_el1/elr_el1, EC==0x15=SVC) — это firebloom/exception-рантайм.

Условие успеха транзакции (FUN_00106d50, цикл до 8 раз):
- FUN_001073c4() = `DAT_7002abd2` = байт[+2] буфера ответа @0x7002abd0 = **op ответа (resp[2])**;
- FUN_001073d0(0) = `descriptor[idx=0]+1` = **ожидаемый op = req+100** (для PING idx0 = 101);
- цикл: `do { transact(); r=resp[2]; e=expected; } while (r==e)`; успех = ПОЛНЫЕ 8 итераций,
  ранний выход (r!=e или ошибка транзакции) → return −1 → вызывающий паникует.

Почему падает у нас: заглушка кладёт ответ в ПЛОСКИЙ MMIO 0x100, а AP читает resp[2] из
RAM-буфера 0x7002abd0, куда 16 байт ответа должен положить recv-путь ЧЕРЕЗ КОЛЬЦО
(FUN_00106820 vtable → FUN_0010d5c4 читает из obj[0]+entry). Наш resp[2] туда не попадает ⇒
r!=e ⇒ транзакция «не завершается» ⇒ паника → reboot.

### Что делать в vr-sep-mbox.c (конкретно)
Отдавать ответ НЕ в плоские регистры, а по пути, который читает FUN_0010d5c4:
1. положить 16-байтный ответ с [0]=0xFF, [2]=req_op+100 туда, откуда recv копирует (obj[0]+entry[2]);
2. в статус-слове (obj[0]+entry[0]) снять bit16 (готово), не ставить биты 0xC0000 (ошибка);
3. поднять IRQ SEP→AP; обеспечить, что PING (idx0) отвечает 101 стабильно 8 раз подряд.
Нужно снять точные entry[0]/entry[2]/индекс кольца (setup мейлбокса) — следующий заход.

### ФИКС РАБОТАЕТ: прошли SEP GENERATE_NONCE (vr-sep-mbox.c)
Две правки ответчика:
1. op3 (GENERATE_NONCE): resp.data32 (resp[4:8]) = **0xa0** — AP (FUN_00106be4/FUN_001073b8)
   требует это значение, иначе пропускает чтение nonce и паникует.
2. op4 (READ_NONCE): resp.data32 = ненулевой чанк (0xA5A5_0000|tag). AP тянет 20-байтный nonce
   по 4 байта × 5 транзакций и требует OR всех 20 байт != 0.
Результат: totalTB 211k→236k, distinct 3073→3719, первый abort УЕХАЛ с мейлбокса (0x106cd0) на
новую функцию **0x1093d8** (TB#235540). FUN_00106be4 отработал один раз рано (TB#209686) — т.е.
GENERATE_NONCE ПРОЙДЕН. Барьер сместился дальше по boot.

### Следующий барьер: ~0x1093d8
Первый abort теперь из функции 0x1092xx-0x1093xx (prev TB: 0x109364/0x1093b4/0x1093d8,
0x108680/0x107fa4/0x10810c). Декомпилировать её и понять новую проверку (возможно снова SEP:
op5 BOOT_TZ0 / op6 BOOT_IMG4, либо IMG4-манифест следующего образа).

### Разбор нового барьера (FUN_001092a0 / FUN_00107f10)
FUN_001092a0 инициализирует 12 очередей (0x70 б каждая @DAT_001476b8+0x30), затем:
`if (!(FUN_00107f10(DAT_7002b999)&1) && !(DAT_7002b999&1)) FUN_0010550c();` — abort, если проба
подсистемы не удалась. FUN_00107f10 поднимает структуру @DAT_001476b0 (флаги 0x100000|6 / |2) и
гейтится на **FUN_0010d8c8()==0**. Это следующая подсистема (вероятно 2-й IOP/мейлбокс или SEP
CONTROL-канал, ep=0x00). Следующий заход: декомпилировать FUN_0010d8c8 + FUN_00108698/FUN_001094ac
(колбэк) и понять, чего она ждёт (снова SEP-обмен? op5/op6? или новый MMIO-девайс).


### ВЗЯТ БАРЬЕР virtio-usb / DEVID_USB (FUN_0010d8c8, 0x30100000)
Анализ и фикс:
1. `FUN_0010d8c8` обращается к BDIF-устройству `DEVID_USB` (0x30100000).
   На смещении +0x4 (`REG_CFG`) драйвер проверяет соответствие с аргументом w4 = `0x1a01` (`cmp w0, w24`).
   В `bdif.c` отдавался константный `REG_CFG_ACTIVE = 2`, что вызывало ошибку `0x40200003` и возврат -3.
   Решение: для `devid == DEVID_USB` отдавать `0x1a01`.
2. После успешной регистрации USB-устройства запускается фоновый поток-обработчик `FUN_0010829c`.
   Он запрашивает смещение +0x10 (`REG_BUSY`). В `bdif.c` для всех устройств отдавалось `1` (`REG_BUSY_READY`).
   Воркер трактовал это как флаг наличия принятого USB-пакета, вычитывал нулевой буфер из RAM и паниковал
   на `0x108608` из-за неверного типа пакета (byte[5] == 0).
   Решение: для `devid == DEVID_USB` возвращать 0 на `REG_BUSY`, когда нет входящих пакетов.

Результат:
- Полное устранение паник (`panic TB# None`).
- Выход гостя в кооперативный цикл многозадачности (`period 78 TB`, свыше 3.8 млн TB).
- Началось чтение блоков `aux.img` (DEVID_AUX) по 512 байт через `vblk_read`.
