...
static void arm_cpu_do_interrupt_aarch64(CPUState *cs)
{
    ...
    bool genter = false;
    bool from_gl = arm_is_guarded(env); /* GL0 (TXM) or GL1 (SPTM) */
    ...
    /* an exception taken from guarded mode stays guarded: GL0 -> GL1 */
    if (from_gl && cur_el < new_el) {
        env->gxf.gxf_status_el[cur_el] &= ~1;
    }
    env->gxf.gxf_status_el[new_el] |= genter || from_gl;
    env->aarch64 = true;
