...
## Обновление 25.09 (89) — 🎉 ПРОРЫВ #2: gigalocker/data-protection обойдён → 10 boot-тасков, том Data монтируется

**Стена gigalocker пробита патчем userspace-бинаря.** `/usr/libexec/init_data_protection` = **симлинк на `/usr/libexec/seputil`** (мульти-тул AppleSEPUtil). seputil делает gigalocker-init через SEP; без реального SEP падает (`Failed to initialize gigalocker: 2`, ENOENT на `/private/xarts/<UUID>.gl`), процесс крашится → boot застревал.

**Анализ:** извлёк seputil из iPhone-OS DMG (`tools/extract_idp.sh` — монтирует ОТДЕЛЬНЫЙ `043-53486-120.dmg` RO, безопасно). Дизасм показал: gigalocker-логика в огромной функции 0x100001c68 (main); success-путь чейнит НОВЫЕ SEP-вызовы → чистого «успеха» без SEP нет. FS-тома **unencrypted** → фейковый data-protection downstream безопасен.

**Фикс (exit-0 патч + re-sign):** эпилог main @0x100003b18 `mov x0, x28` (возврат = код ошибки) → `mov x0, #0` (всегда возврат 0). `tools/resign_dyld.py` (общий) пере-подписал: пере-хэш страницы 3 в CodeDirectory, новый cdhash `8ada3251...` (old `b27d52c0...`). Добавил cdhash в TC (`tc_append.py` → merged7, 3816 записей), собрал StaticTrustCache (`build_static_tc.py`), заинжектил в root2.big.img Preboot (`inject_static_tc.sh` IMG=root2.big.img). Заменил seputil в System-томе на пропатченный (`tools/replace_idp.sh`; init_data_protection→симлинк, cp пишет сквозь него в seputil). Проверка `tools/verify_idp.sh`.

**Результат (boot_dp.console, TC=merged7):** init_data_protection всё ещё логирует внутреннюю ошибку gigalocker, но **выходит 0** → launchd проходит data-protection и гонит **10 boot-тасков** (было 4): exclaves-boot, fsck, mount-phase-1, data-protection✓, **detect-installed-roots, select-boot-mode, commit-boot-mode, restore-datapartition, finish-obliteration, mount-phase-2**. Спавнились mobile_obliterator, HorizonSetup (0 крашей дочерних). **mount-phase-2 монтирует том DATA** (/private/var, disk1s3, role 64, unencrypted, mount_apfs pid 10).

**🔴 НОВАЯ СТЕНА:** краш во время mount Data. Симптом до краша: `mount: failed to migrate Media Keys, error = c002` (keystore/AppleKeyStore-ошибка — media keys управляются keystore/keybag, которые мы фейкнули). Последний EL0-контекст = mount_apfs (pid 10) на syscall 167 (mount) → процесс крашится (Corefile magic 0x0). Похоже, **keystore/keybag стена**: монтирование/доступ к Data триггерит keystore-операцию (media keys), которая падает без реального keystore. Продолжение data-protection/SEP-фронтира. Next: разобрать что крашится при mount Data (mount_apfs или keystore-демон), нужен ли keybag для unencrypted-тома, обойти media-keys (error c002) — kernel-side AppleKeyStore или userspace.

**⚠️ Побочный риск патча:** seputil main всегда возвращает 0 — это влияет на ВСЕ вызовы seputil в boot'е (sepfw-load-at-boot тоже). sepfw-load отработал ДО патча-эффекта (SEP fw preloaded), возврат 0 для успешной операции безвреден. Если позже нужен non-zero код от seputil — пересмотреть (сузить патч до gigalocker-ветки).

## Обновление 25.09 (90) — стена Data-mount локализована: NULL-дереф в APFS из-за keystore c002

Добавил в эмулятор лог EL0/EL1-синхронных абортов (`overlay/target/arm/helper.c` `arm_log_exception`, gate `VR_EXCLOG`, → `exc.log`: тип/EL/pc/far/esr). Прогон (TC=merged7, патченный seputil) на крашe mount Data:
- **Фатал = EL1 DABT в ЦИКЛЕ: `pc=0xfffffe0031a505e4 far=0xe00 esr=0x96000006`** (EC=0x25 Data Abort same-EL, DFSC=6 L2 translation fault, чтение). **far=0xe00 = NULL+0xe00 — разыменование NULL** в APFS-kext (рантайм-база 0xfffffe0031xxxxxx, отдельный kext в fileset, со своим KASLR-слайдом ≠ main kernel 0xfffffe0007xxx).
- Соседние EL1-аборты: 0x31a000f0/0x31a002d0/0x31bb8a90 (far=0xfffffecff… высокие адреса). Доминирует цикл 0x31a505e4/far=0xe00.
- Причина: `mount: failed to migrate Media Keys, error = c002` — keystore не отдал media-keys тома Data → crypto/keybag-объект = NULL → APFS при mount дерефит [null+0xe00]. **Keystore/keybag стена** (media keys обёрнуты SEP; наш SEP застаблен).
...
## Обновление 26.09 (95) — 🎯 ПЕРВИЧНАЯ ПРИЧИНА mount-Data паники: "unencrypted data volume is not allowed"

Раскрыл через регистры ПЕРВОГО far=0xe00 дампа (не рекурсивного — 2-е+ дампы это паника-в-панике про краш самого обработчика). x19 первого дампа → link 0x7634920 (APFS __cstring) → **`"unencrypted data volume is not allowed" @%s:%d`** (APFS-ассерт с file:line).

**СУТЬ СТЕНЫ (окончательно):** это НЕ глубокий keystore-null-дереф — это **явный APFS-ассерт/политика: незашифрованный Data-том запрещён.** Наш Data-том (disk1s3) создан пустым/unencrypted (flags 0x1), а iOS требует Data с data-protection-шифрованием → APFS паникует «unencrypted data volume is not allowed» → сломанный на null-SPTM панич-обработчик → зависание. (media-keys c002 — следствие того же: нет шифрования/ключей.)

**Техника раскрытия:** на far=0xe00 краше читать строки по указателям из регистров ПЕРВОГО дампа (link = rt − slide, slide = vbar − 0xfffffe0008a5f000), из KC-файла офлайн (rd_first.py). Строка "unencrypted data volume..." @ link 0xfffffe0007634920 (file 0x630920).

**ФИКС (следующий заход — гораздо обходимее, чем думали):**
1. **Обойти ассерт** — найти в APFS __text код, ссылающийся на строку 0x7634920 (xref капстоном не нашёлся — ассерт через макрос/дескриптор-таблицу; искать иначе: по вызову panic-обёртки рядом с проверкой crypto-флага тома, ИЛИ дизасм handle_mount после «setting dev block size»), затем VR_B/VR_NOP обойти проверку → unencrypted Data смонтируется.
