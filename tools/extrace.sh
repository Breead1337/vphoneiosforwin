...
# usage (WSL): AUX=aux.test ROOT=root.img T=120 bash extrace.sh   -> ~/vrwork/ex.log, uart in vr.uart
...
awk -v N=${N:-60} '
  /^Taking exception/ { svc = ($0 ~ /\[SVC\]/); if (!svc && shown < N) { blk = 1; shown++ } else blk = 0 }
  /unimplemented|unsupported/ { if (u[$0]++ < 3) print }
  blk { print }
  /with ELR/ && !svc { h[$NF]++ }
