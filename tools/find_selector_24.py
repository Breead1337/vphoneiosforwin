...
# TXM functions are at 0xfffffe0008ab1500, 0xfffffe0008ab1580
target_txm = [0xfffffe0008ab1500, 0xfffffe0008ab1580]

print("Searching for BL to 0x8ab1500 or 0x8ab1580...")
for i in range(0, len(s_data), 4):
    ...
    if (insn_val & 0xfc000000) == 0x94000000:
        ...
        pc = text_exec_va + i
        target = pc + (imm26 << 2)
        if target in target_txm:
            print(f"Call to 0x{target:x} at 0x{pc:x}:")
            dis_start = max(0, i - 24)
            dis_end = min(len(s_data), i + 24)
