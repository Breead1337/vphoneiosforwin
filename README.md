...
> ⚠️ Это **ресёрч**, а не продукт. На сегодня загружается настоящий `AVPBooter` Apple на x86,
> исполняется настоящий `LLB` (Microkernel iBoot-13822), инициализируются `SPTM` и `TXM` под `GXF`,
> и ядро iOS (`XNU`) загружается в `IOKit` — подробности пути и журнал вех в [`NOTES.md`](NOTES.md).
